...
// This file contains the shared, build-tag-independent (non-Windows) logic for
// loading ONNX Runtime and setting up the environment. The only piece that
// differs between the default build and the "static" build tag is how the ORT
// symbols are obtained; that lives in loadORTHandle(), which is declared in the
// tag-specific file (setup_env.go for the default build, setup_env_static.go for
// the "static" build tag). Keeping only loadORTHandle tag-specific avoids
// duplicating the rest of the environment-setup logic between the two builds.
